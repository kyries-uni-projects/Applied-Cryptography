	...
	return (
		<div className="space-y-6">
			<div className="flex justify-between items-center">
				...
				<button onClick={fetchCerts} className="btn btn-ghost btn-sm">
					Làm mới
				</button>
			</div>
								...
								<tr key={c.id} className="hover">
									...
									<td>
										<div className="flex gap-1">
											...
											{c.status === "ACTIVE" && (
												<>
													<button className="btn btn-warning btn-xs" onClick={() => handleRenew(c.id)} disabled={actionLoading === c.id}>
														Gia hạn
													</button>
													<button className="btn btn-error btn-xs" onClick={() => handleRevoke(c.id)} disabled={actionLoading === c.id}>
														Thu hồi
													</button>
												</>
											)}
					...
					<div className="modal-box max-w-2xl">
						...
						<div className="grid grid-cols-2 gap-3 text-sm">
							<div>
								...
								<p className="font-mono break-all">{viewCert.subjectDN}</p>
							</div>
							<div>
								<p className="text-xs text-base-content/50">Issuer</p>
								<p className="font-mono break-all">{viewCert.issuerDN}</p>
							</div>
							<div>
								<p className="text-xs text-base-content/50">Serial</p>
								<p className="font-mono break-all">{viewCert.serialNumber}</p>
							</div>
							<div>
								<p className="text-xs text-base-content/50">Status</p>
								<span className={`badge ${viewCert.status === "ACTIVE" ? "badge-success" : "badge-error"}`}>{viewCert.status}</span>
							</div>
						</div>
						<div className="divider">PEM</div>
						<pre className="bg-base-300 rounded-box p-4 text-xs max-h-48 overflow-auto whitespace-pre-wrap break-all">{viewCert.certPem}</pre>
						<div className="modal-action">
							<a
								href={`data:application/x-pem-file;charset=utf-8,${encodeURIComponent(viewCert.certPem)}`}
								download={`cert-${viewCert.serialNumber.slice(0, 8)}.pem`}
								className="btn btn-outline btn-sm"
							>
								Download
							</a>
							<button className="btn" onClick={() => setViewCert(null)}>
								Đóng
							</button>
						</div>
					</div>
