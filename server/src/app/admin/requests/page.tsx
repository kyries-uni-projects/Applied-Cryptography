	...
	return (
		<div className="space-y-6">
			<div className="flex justify-between items-center">
				...
				<button onClick={fetchRequests} className="btn btn-ghost btn-sm">
					Làm mới
				</button>
			</div>
							...
							{requests.map((r) => (
								<tr key={r.id} className="hover">
									...
									<td className="text-xs">
										{r.keyPair.label} ({r.keyPair.keyLength}bit)
									</td>
									<td>{badge(r.status)}</td>
									<td className="text-xs">{new Date(r.createdAt).toLocaleString("vi-VN")}</td>
									<td>
										<div className="flex gap-1">
											<button className="btn btn-ghost btn-xs" onClick={() => setViewCsr(r.csrPem)}>
												Xem
											</button>
											{r.status === "PENDING" && (
												<>
													...
													<button className="btn btn-error btn-xs" onClick={() => setRejectId(r.id)}>
														Từ chối
													</button>
												</>
											)}
			...
			{rejectId && (
				<dialog className="modal modal-open">
					...
					<form method="dialog" className="modal-backdrop">
						<button onClick={() => setRejectId(null)}>close</button>
					</form>
				</dialog>
			)}
			...
			{viewCsr && (
				<dialog className="modal modal-open">
					<div className="modal-box max-w-2xl">
						...
						<pre className="bg-base-300 rounded-box p-4 text-xs mt-4 max-h-96 overflow-auto whitespace-pre-wrap break-all">{viewCsr}</pre>
						<div className="modal-action">
							<button className="btn" onClick={() => setViewCsr(null)}>
								Đóng
							</button>
						</div>
					</div>
					<form method="dialog" className="modal-backdrop">
						<button onClick={() => setViewCsr(null)}>close</button>
					</form>
				</dialog>
			)}
		</div>
