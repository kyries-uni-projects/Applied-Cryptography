								...
								<tr key={k.id} className="hover">
									...
									<td className="font-mono text-xs">
										{k.blockchainAddress ? (
											<div className="flex items-center gap-2">
												<span>
													{k.blockchainAddress.slice(0, 8)}...{k.blockchainAddress.slice(-6)}
												</span>
												<button
									...
									<td>
										<div className="flex gap-1">
											<button className="btn btnghost btn-xs" onClick={() => setViewKey(k)}>
												Xem
											</button>
											<a
												href={`data:application/x-pem-file;charset=utf-8,${encodeURIComponent(k.publicKeyPem)}`}
												download={`${k.label}-public.pem`}
												className="btn btn-outline btn-xs"
											>
												Public
											</a>
											<a
												href={`data:application/x-pem-file;charset=utf-8,${encodeURIComponent(k.privateKeyPem)}`}
												download={`${k.label}-private.pem`}
							...
							{keys.length === 0 && (
								<tr>
									<td colSpan={5} className="text-center py-8 text-base-content/50">
										Chưa có cặp khóa nào. Tạo mới ở trên.
									</td>
								</tr>
							)}
						</tbody>
				...
				<dialog className="modal modal-open">
					<div className="modal-box max-w-2xl">
						<h3 className="font-bold text-lg">
							{viewKey.label} ({viewKey.keyLength} bits)
						</h3>
						<div className="space-y-4 mt-4">
							<div>
								<p className="text-xs font-semibold text-base-content/50 uppercase mb-1">Public Key</p>
								<pre className="bg-base-300 rounded-box p-4 text-xs max-h-40 overflow-auto whitespace-pre-wrap break-all">{viewKey.publicKeyPem}</pre>
							</div>
							<div>
								<p className="text-xs font-semibold text-base-content/50 uppercase mb-1">Private Key</p>
								<pre className="bg-base-300 rounded-box p-4 text-xs max-h-40 overflow-auto whitespace-pre-wrap break-all">
									{viewKey.privateKeyPem}
								</pre>
							</div>
						</div>
						<div className="modal-action">
							<button className="btn" onClick={() => setViewKey(null)}>
								Đóng
							</button>
						</div>
					</div>
