					...
					<div className="card-body">
						<form onSubmit={handleSubmit} className="space-y-4">
							<div className="flex flex-col gap-1">
								<label className="label font-medium">Chọn chứng chỉ cần thu hồi *</label>
								<select className="select select-bordered w-full" value={certificateId} onChange={(e) => setCertificateId(e.target.value)}>
									{certs.map((c) => (
										<option key={c.id} value={c.id}>
											{c.request?.domain || c.subjectDN} (SN: {c.serialNumber.slice(0, 8)}...)
										</option>
									))}
								</select>
							</div>
							<div className="flex flex-col gap-1">
								<label className="label font-medium">Lý do thu hồi *</label>
								<textarea
