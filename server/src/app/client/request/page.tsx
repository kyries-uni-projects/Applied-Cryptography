...
export default function RequestPage() {
	...
	useEffect(() => {
		fetch("/api/client/keys")
			...
			.then((d) => {
				setKeys(d);
				if (d.length > 0) setKeyPairId(d[0].id);
			})
						...
						<form onSubmit={handleSubmit} className="space-y-4">
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								<div className="flex flex-col gap-1">
									<label className="label font-medium">Cặp khóa *</label>
									<select className="select select-bordered w-full" value={keyPairId} onChange={(e) => setKeyPairId(e.target.value)}>
										{keys.map((k) => (
											<option key={k.id} value={k.id}>
												{k.label} ({k.keyLength} bits)
											</option>
										))}
									</select>
								</div>
								<div className="flex flex-col gap-1">
									<label className="label font-medium">Tên miền Website (CN) *</label>
									<input
										type="text"
