					...
					<div className="modal-box max-w-2xl">
						...
						<div className="grid grid-cols-2 gap-3 text-sm">
							<div>
								...
								<p className="font-mono break-all text-xs">{viewCert.subjectDN}</p>
							</div>
							<div>
								<p className="text-xs text-base-content/50">Issuer</p>
								<p className="font-mono break-all text-xs">{viewCert.issuerDN}</p>
							</div>
							<div>
								<p className="text-xs text-base-content/50">Hiệu lực từ</p>
								<p>{new Date(viewCert.notBefore).toLocaleString("vi-VN")}</p>
							</div>
							<div>
						...
						<pre className="bg-base-300 rounded-box p-4 text-xs max-h-40 overflow-auto whitespace-pre-wrap break-all">{viewCert.certPem}</pre>
